...
namespace IOFrame{
     ...
     * */
    class routeHandler extends abstractDBWithCache
    {
        ...
        function addRoutes(array $inputs, array $params = []){
            ...
            foreach($inputs as $index=>$input){
                $input[1] = $safeStr ? str2SafeStr($input[1]) : $input[1];
                //Set default
                if(!isset($input[3]))
                    $input[3] = null;
        ...
        function updateRoutes(array $inputs, array $params = []){
            ...
            foreach($inputs as $inputIndex => $inputArray){
                ...
                for($i = 1; $i<5; $i++){
                    ...
                    elseif($inputArray[$i] == null)
                        ...
                        $inputArray[$i] = ($safeStr && $field == 'Route') ? str2SafeStr($inputArray[$i]) : $inputArray[$i];
            ...
            if(count($inputs)>0){
                ...
                if($useCache){
                    if(!$test)
                        $this->redisHandler->call('del',['ioframe_route_id_'.$inputArray[0]]);
                    if($verbose)
        ...
        function deleteRoutes(array $IDs, array $params = []){
            ...
            if($useCache){
                foreach($IDsToDelete as $ID){
                    ...
                    if(!$test)
                        $this->redisHandler->call('del',['ioframe_route_id_'.$ID]);
                }
            }
        ...
        function getRoutes(array $IDs = [], array $params = []){
            ...
            if($safeStr){
                foreach($res as $id=>$route){
                    if(is_array($route)){
                        $res[$id]['Route'] = safeStr2Str($route['Route']);
                    }
                }
            }
        ...
        function getMatches(array $matches = [], array $params = []){
            ...
            if($safeStr){
                foreach($res as $matchName=>$match){
                    if(is_array($match)){
                        $res[$matchName]['URL'] = safeStr2Str($match['URL']);
                    }
                }
            }
