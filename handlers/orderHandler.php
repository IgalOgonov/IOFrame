...
namespace IOFrame{
    ...
    class orderHandler extends abstractDBWithCache
    {
        ...
        /**
         * @var string The cache name - used as a prefix for the full cache name - $cacheName.'_order'. Defaults to $name.
         */
        ...
        /** Tries to get the global plugin order.
         * @param array $params
        ...
        protected function getGlobalOrder(array $params = []){
            ...
            $order = $this->redisHandler->call('get',$this->cacheName.'_order');
            ...
            if($order === false && $this->tableName !== null){
                ...
                $order = $this->sqlHandler->selectFromTable(
                    ...
                    [
                        [$this->columnNames[0],$this->name.'_order','=']
                    ],
                    [$this->columnNames[1]],
                    $params
                );
                ...
                if($order){
                    if(!$test)
                        $this->redisHandler->call('set',[$this->cacheName.'_order',$order,$cacheTTL]);
                    if($verbose)
        ...
        protected function updateGlobalOrder(string $newOrder, $params = []){
            ...
            $res = (
                $this->sqlHandler->insertIntoTable(
                    $this->sqlHandler->getSQLPrefix().$this->tableName,
                    [$this->columnNames[0],$this->columnNames[1]],
                    [[[$this->name.'_order','STRING'], [$newOrder,'STRING']]],
            ...
            if($useCache && $res){
                if(!$test)
                    $this->redisHandler->call('del',[$this->cacheName.'_order']);
                if($verbose)
        ...
        function pushToOrderMultiple(array $names, $params = []){
            ...
            if($order === ''){
                if($local){
                    ...
                    if($this->updateGlobalOrder($namesString,$params))
                        foreach($names as $name){
                            $res[$name] = 0;
                        }
                    return $res;
            ...
            elseif($order === false){
                ...
                if($local){
                    ...
                    if($this->updateGlobalOrder($order,$params))
                        foreach($names as $name){
                            $res[$name] = 0;
                        }
                    return $res;
