...
function checkLoggedIn(pathToRoot,trustLocalStorage=false){
    ...
    /*
    if ((typeof(Storage) !== "undefined") && trustLocalStorage) {
        if(localStorage.getItem('sesInfo') == '[]')
            res = false;
        else{
            let sesInfo = JSON.parse(localStorage.getItem('sesInfo'));
            if(sesInfo['logged_in']=true)
                res = sesInfo['Username'];
        }
        ...
        var request = $.ajax({
            url:  pathToRoot+"_siteAPI\/sessionAPI.php"
            ,data: "logged_in"
            ,headers: header
        });

        request.done(function(response) {
            //We are logged in, unless it's []
            if(response!="[]")
                res = JSON.parse(response)['logged_in'];
            else
                res = false;
            if(dfd!==undefined)
                dfd.resolve(res);
