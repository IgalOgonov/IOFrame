...
namespace IOFrame{
     ...
     * */
    class mailHandler extends abstractDBWithCache{
        ...
        function createSecToken($mail, $duration=1, $length=30, $override=false){
            $dbres = $this->sqlHandler->selectFromTable('MAIL_AUTH',['Name',$mail,'='],[],[]);
            //If no token for $mail existed, insert new values
            if(!is_array($dbres)){
                $token = GeraHash($length);
         ...
         *
         * */
        function verfySecToken($token, $mails){
            is_json($mails)?
                $cond = 'MailAPIGlobal'
                :
                $cond = $mails;
            $dbres = $this->sqlHandler->selectFromTable('MAIL_AUTH',['Name',$cond,'='],[],[]);
            if(!is_array($dbres))
                return 1;
            else
                $dbres = $dbres[0];
        ...
        function setTemplate($templateID, $convertFromSafeSTR = true){
            if (!preg_match('/^[0-9]*$/', $templateID)) {
                ...
                try{
                    $mailTemplate = $this->sqlHandler->selectFromTable('MAIL_TEMPLATES',['ID',$templateID,'='],[],['noValidate'=>true]);
                    if(is_array($mailTemplate)){
                        $mailTemplate = $mailTemplate[0];
                        if($convertFromSafeSTR)
                            $mailTemplate['Content'] = safeStr2Str($mailTemplate['Content']);
        ...
        function sendMailAsync( $address, $subject, $secToken, $mailerName =['',''],
            ...
            curl_setopt($ch, CURLOPT_URL,"http://".$_SERVER['SERVER_NAME'].$this->settings->getSetting('pathToRoot')."/_siteAPI/mailAPI.php");
