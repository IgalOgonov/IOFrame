...
class IOFrameHandler extends AbstractProcessingHandler{
    ...
    /* @var IOFrame\settingsHandler as defined in IOFrame
     * */
    private $settings;
    /* @var IOFrame\sqlHandler as defined in IOFrame
     * */
    private $sqlHandler;
    /* @var IOFrame\fileHandler as defined in IOFrame
     * */
     ...
     *
     * @param  IOFrame\settingsHandler  $settings  IOFrame settings handler
     * @param  IOFrame\sqlHandler $sqlHandler  IOFrame database handler. Can be left null and built from $settings.
    ...
    public function __construct(IOFrame\settingsHandler $settings, IOFrame\sqlHandler $sqlHandler = null, $opMode ='local',
                                $timeBasedFolders = true, $checkInit = true, $target = 'ACTIVE', $level = Logger::DEBUG, $bubble = true){
        ...
        elseif($opMode == 'local'){
            ...
            if(preg_match_all('/\w|.|\//',$target)<strlen($target) || strlen($target)>100 || $target == 'ACTIVE'){
                $target =  $this->timeBasedFolders? '_siteFiles/logs/'.((int)((time()%3600)/60)).'/logs.txt' : '_siteFiles/logs.txt';
            }
    ...
    protected function writeToFile(array $record){
        if($this->fileHandler == null){
            $this->fileHandler = new IOFrame\fileHandler();
        }
        ...
        $this->fileHandler->writeFileWaitMutex(
            ...
            ['append' => $this->appendToLog, 'useNative' => $this->useNativeLock]
        );
    }
