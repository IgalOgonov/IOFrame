...
namespace IOFrame{
    ...
    function initDB(settingsHandler $localSettings){
        $userSettings = new settingsHandler(getAbsPath().'/_siteFiles/userSettings/');
        $siteSettings = new settingsHandler(getAbsPath().'/_siteFiles/siteSettings/');
        ...
        try {
             ...
             * */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."MAIL_TEMPLATES(
            ...
            $content = "Hello!<br> To activate your account on ".$siteSettings->getSetting('siteName').", click <a href=\"http://".$_SERVER['HTTP_HOST'].$localSettings->getSetting('pathToRoot')."_siteAPI/userAPI.php?action=regConfirm&id=%%uId%%&code=%%Code%%\">this link</a><br> The link will expire in ".$userSettings->getSetting('mailConfirmExpires')." hours";
            ...
            $content = "Hello!<br> To change your mail on ".$siteSettings->getSetting('siteName').", click <a href=\"http://".$_SERVER['HTTP_HOST'].$localSettings->getSetting('pathToRoot'). "_siteAPI/userAPI.php?action=mailReset&id=%%uId%%&code=%%Code%%\">this link</a><br> The link will expire in ".$userSettings->getSetting('mailConfirmExpires')." hours";
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."USERS (
                                                              ID int PRIMARY KEY NOT NULL AUTO_INCREMENT,
            ...
            $makeLoginIndex = $conn->prepare("CREATE INDEX loginIndex ON ".$prefix.
                "USERS (Email);");
            try{
                $makeTB->execute();
             ...
             * */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."LOGIN_HISTORY (
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."USERS_EXTRA (
                                                              ID int PRIMARY KEY,
            ...
            $makeIndex1 = $conn->prepare("CREATE INDEX Banned ON ".$prefix.
                "USERS_EXTRA (Banned_Until);");
            //We will often filter by Suspicious Activity
            $makeIndex2 = $conn->prepare("CREATE INDEX Suspicious ON ".$prefix.
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."USERS_AUTH (
                                                              ID int PRIMARY KEY,
            ...
            $makeIndex = $conn->prepare("CREATE INDEX LastChangedIndex ON ".$prefix.
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."GROUPS_AUTH (
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."ACTIONS_AUTH (
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."USERS_ACTIONS_AUTH (
                                                                ID int,
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."GROUPS_ACTIONS_AUTH (
             ...
             * Auth_Group - foreign key - name of the group
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."USERS_GROUPS_AUTH (
                                                                ID int,
                                                                Auth_Group varchar(256),
                                                                FOREIGN KEY (ID)
                                                                REFERENCES ".$prefix."USERS(ID)
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."MAIL_AUTH (
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."IP_LIST (
                                                              IP varchar(45) NOT NULL,
            ...
            $makeIndex = $conn->prepare("CREATE INDEX ExpiresIndex ON ".$prefix.
             ...
             * expires - the date until the effect should last. Stored in Unix timestamp, usually.
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."IPV4_RANGE (
                                                              IP_Type BOOLEAN NOT NULL,
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."IP_LIST_META (
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."IPV4_RANGE_META (
                                                              settingKey varchar(255) PRIMARY KEY,
                                                              settingValue varchar(255) NOT NULL
                                                              ) ENGINE=InnoDB DEFAULT CHARSET = utf8;
                                                              ");
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."IP_EVENTS (
                                                              IP varchar(45) NOT NULL,
             ...
             * Sequence_Expires   - The time when current event aggregation sequence expires, and a new one may begin.
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."USER_EVENTS (
                                                              ID int NOT NULL,
                                                              Event_Type BIGINT UNSIGNED NOT NULL,
                                                              Sequence_Expires varchar(14) NOT NULL,
                                                              Sequence_Start_Time varchar(14) NOT NULL,
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."EVENTS_RULEBOOK (
             ...
             */
            $makeTB = $conn->prepare("CREATE TABLE ".$prefix."DB_BACKUP_META(
                                                              ID int PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
// INITIALIZE OBJECT_CACHE
             ...
             * Group_Size - Integer, size of the group. The API handling SQL statements should DELETE groups
             *              instead of reducing their size to 0.
             * Owners     - Owners of the group. Starts with the owner of the first object that went in, can be expanded.
             * Min_Modify_Rank - the minimum rank at which every user can modify the whole group.
             * Min_View_Rank -  the minimum rank at which every user can view the whole group. -1 is "free to view".
             ...
             * Page_Name - Varchar(500), the route from the root of the site to the page. If the page is at, for example,
